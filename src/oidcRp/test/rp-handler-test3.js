...
describe('RPHandlerTier2', function () {
    ...
    it('Test refresh access token', function testSlash(done) {        
        ...
        let authResponse = rph.finalizeAuth(client, session['iss'], response.claims);
        ...
        .expect(200, function(err, response){
          ...
          .expect(200, function(err, response){
            ...
            let server3 = makeServer(at.toJSON());
            ...
            .get('/')
            .expect(200, function(err, response){
                let resp = rph.refreshAccessToken(res['state_key'], client, 'openid email', response);
                assert.deepEqual(resp.claims['access_token'], '2nd_accessTok');
                done();
            });
          });
        });
