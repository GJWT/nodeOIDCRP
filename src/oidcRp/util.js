...
class Util {
  ...
  getOrPost(uri, method, req, contentType, accept, kwargs) {
    ...
    if (reqActions.indexOf(method) !== -1) {
      ...
      if (Object.keys(kwargs).indexOf('headers') !== -1) {
        kwargs['headers'] = Object.assign(headerExt, kwargs['headers']);
      } else {
  ...
  getDeserializationMethod(response){
    let cType = '';
    ...
    if (this.matchTo("application/json", cType) || this.matchTo("application/jrd+json", cType)){
      ...
      deserMethod = 'urlencoded';
    }else if (this.matchTo('text/plain', cType) || this.matchTo('test/html', cType)){
      deserMethod = '';
    }else{
      deserMethod = '';
    }
